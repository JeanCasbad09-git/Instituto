...
public class Frm_ConsultaRegistro extends JInternalFrame implements ActionListener, Runnable, MouseListener	{
	...
	public Frm_ConsultaRegistro() {
		...
		lblAlumno = new JLabel("CONSULTA REGISTRO");
		...
		cboConsultaPor.setModel(new DefaultComboBoxModel(new String[] {"SELECCIONE TIPO DE FILTRO", "C\u00F3digo Registro", "C\u00F3digo Alumno", "C\u00F3digo Curso"}));
	...
	protected void do_btnConsultar_actionPerformed(ActionEvent e) {
		...
		if(cboConsultaPor.getSelectedIndex()>=0 && cboConsultaPor.getSelectedIndex()<=3) {
			...
			if(cboConsultaPor.getSelectedIndex()==1) {
				lista=model.listaIdRegistrado(dato);
			}
			if(cboConsultaPor.getSelectedIndex()==2) {
				lista=model.listaIdAlumnoRegistrado(dato);
			}
			if(cboConsultaPor.getSelectedIndex()==3) {
				lista=model.listaIdCursoRegistrado(dato);
			}
		}
		
		if(lista.isEmpty()) {
			mensaje("No se encontró Registro(s) con los datos ingresados");
			listar();
			return;
		}
	...
	private void agregarDatosACBODATOS() {
		...
		case 1: limpiar();
			cboConsultaPor.setSelectedIndex(1);
				lista=model.listaIdRegistrado();
				for (Registro registro : lista) {
					cboDatos.addItem(registro.getIdREGISTRO());
				};break;
		case 2:limpiar();
		cboConsultaPor.setSelectedIndex(2);
		lista=model.listaIdAlumnoRegistrado();
		for (Registro registro : lista) {
			cboDatos.addItem(registro.getIdAlumno());
				};break;
		case 3:limpiar();
		cboConsultaPor.setSelectedIndex(3);
		lista=model.listaIdCurso();
		for (Registro registro : lista) {
			cboDatos.addItem(registro.getIdCurso());
				};break;
		}
		}
