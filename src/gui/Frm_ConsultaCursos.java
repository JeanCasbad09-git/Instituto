...
public class Frm_ConsultaCursos extends JInternalFrame implements ActionListener, Runnable, MouseListener	{
	...
	public Frm_ConsultaCursos() {
		...
		lblAlumno = new JLabel("CONSULTA CURSOS");
		...
		cboConsultaPor.setModel(new DefaultComboBoxModel(new String[] {"SELECCIONE TIPO DE FILTRO", "C\u00F3digo Curso", "Curso", "Ciclo", "Cr\u00E9ditos"}));
	...
	protected void do_btnConsultar_actionPerformed(ActionEvent e) {
		...
		if(cboConsultaPor.getSelectedIndex()>=0 && cboConsultaPor.getSelectedIndex()<=4) {
			...
			if(cboConsultaPor.getSelectedIndex()==1) {
				lista=model.listarIdCurso(dato);
			}
			if(cboConsultaPor.getSelectedIndex()==2) {
				lista=model.listarCurso(dato);
			}
			if(cboConsultaPor.getSelectedIndex()==3) {
				lista=model.listarCiclo(Integer.parseInt(dato));
			}
			if(cboConsultaPor.getSelectedIndex()==4) {
				lista=model.listarCreditos(Integer.parseInt(dato));
			}
		}
		
		if(lista.isEmpty()) {
			mensaje("No se encontró Curso(s) con los datos ingresados");
			listar();
			return;
		}
		for (Cursos x : lista) {
	...
	private void agregarDatosACBODATOS() {
		...
		case 1: limpiar();
			cboConsultaPor.setSelectedIndex(1);
				lista=model.listarIdCurso();
				for (Cursos cursos : lista) {
					cboDatos.addItem(cursos.getIdCurso());
				};break;
		case 2:limpiar();
		cboConsultaPor.setSelectedIndex(2);
		lista=model.listarCurso();
		for (Cursos cursos : lista) {
			cboDatos.addItem(cursos.getCurso());
				};break;
		case 3:limpiar();
